...
export default function GameTarget(props: { player: Player }) {
	...
	const handleClick = (value: number, multiplier: number) => {
		...
		if (
			Number(type) - (scoreAtStartOfTurn + scoreScoredThisTurn + totalScore) <
			0
		) {
		...
		if (currentTurnCount === 2) {
			...
			if (turnScore <= 20) speak(type, 'low', props.player, turnScore)
			else if (turnScore >= 60) speak(type, 'high', props.player, turnScore)
	...
	const handleCancel = () => {
		...
		if (scoreBeforeCancel === Number(type)) {
